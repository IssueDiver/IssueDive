...
class JwtAuthenticationFilterTest {
    ...
    void doFilterInternal_ValidToken_Success() throws ServletException, IOException {
        ...
        given(jwtUtil.getUserEmailFromToken(validToken)).willReturn(email);
        given(jwtUtil.validateToken(validToken, email)).willReturn(true);
        ...
        verify(tokenBlackListService, times(1)).isBlackListed(validToken);
        verify(jwtUtil, times(1)).getUserEmailFromToken(validToken);
    ...
    void doFilterInternal_NoAuthHeader_PassThrough() throws ServletException, IOException {
        // given
    ...
    @Test
    @DisplayName("Bearer 토큰이 아닐 때 필터 통과")
    void doFilterInternal_NotBearerToken_PassThrough() throws ServletException, IOException {
        // given
    ...
    @Test
    @DisplayName("블랙리스트에 있는 토큰으로 인증 실패")
    void doFilterInternal_BlacklistedToken_AuthFails() throws ServletException, IOException {
        // given
        StringWriter stringWriter = new StringWriter();
        PrintWriter writer = new PrintWriter(stringWriter);

        given(request.getHeader("Authorization")).willReturn(bearerToken);
        given(tokenBlackListService.isBlackListed(validToken)).willReturn(true);
        ...
        assertTrue(responseBody.contains("유효하지 않은 토큰입니다"));
        assertTrue(responseBody.contains("\"success\": false"));
    }

    @Test
    @DisplayName("유효하지 않은 JWT 토큰으로 인증 실패")
    void doFilterInternal_InvalidToken_AuthFails() throws ServletException, IOException {
        // given
        StringWriter stringWriter = new StringWriter();
        PrintWriter writer = new PrintWriter(stringWriter);
        ...
        given(tokenBlackListService.isBlackListed(validToken)).willReturn(false);
        given(jwtUtil.getUserEmailFromToken(validToken)).willReturn(email);
        given(jwtUtil.validateToken(validToken, email)).willReturn(false);
        given(response.getWriter()).willReturn(writer);
    ...
    @Test
    @DisplayName("JWT 처리 중 예외 발생 시 에러 응답")
    void doFilterInternal_ExceptionThrown_ErrorResponse() throws ServletException, IOException {
        // given
        StringWriter stringWriter = new StringWriter();
        PrintWriter writer = new PrintWriter(stringWriter);
        ...
        given(request.getHeader("Authorization")).willReturn(bearerToken);
        given(tokenBlackListService.isBlackListed(validToken)).willReturn(false);
        given(jwtUtil.getUserEmailFromToken(validToken)).willThrow(new RuntimeException("JWT parsing error"));
        given(response.getWriter()).willReturn(writer);
    ...
    @Test
    @DisplayName("공개 URL에 대해 필터 적용하지 않음")
    void shouldNotFilter_PublicUrls_ReturnsTrue() throws ServletException {
        // given & when & then
    ...
    @Test
    @DisplayName("보호된 URL에 대해 필터 적용")
    void shouldNotFilter_ProtectedUrls_ReturnsFalse() throws ServletException {
        // given & when & then
        given(request.getRequestURI()).willReturn("/issues");
        assertFalse(jwtAuthenticationFilter.shouldNotFilter(request));

        given(request.getRequestURI()).willReturn("/auth/users/1");
        assertFalse(jwtAuthenticationFilter.shouldNotFilter(request));

        given(request.getRequestURI()).willReturn("/labels");
        assertFalse(jwtAuthenticationFilter.shouldNotFilter(request));
    }

    @Test
    @DisplayName("이미 인증된 상태에서는 재인증하지 않음")
    void doFilterInternal_AlreadyAuthenticated_SkipAuth() throws ServletException, IOException {
        // given
        given(request.getHeader("Authorization")).willReturn(bearerToken);

        // SecurityContext에 이미 인증 정보가 있는 상태를 시뮬레이션
    ...
    @Test
    @DisplayName("null URI 처리 - 필터 적용")
    void shouldNotFilter_NullUri_ReturnsFalse() throws ServletException {
        // given
